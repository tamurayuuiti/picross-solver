...
//
// アーキテクチャ概要:
// - エラー表示・ジャンプ・ハイライトは、App.tsx の useErrorFocus が一元管理する
//   HintErrorFocus（行/列位置 + 発生源 + requestId）を受け取って処理する。
//   発生源がテキスト入力欄・SolverPanelの矛盾アラート・盤面自身のいずれでも
//   同じ処理（スクロール+一時ハイライト）になる。
// - 強調表示はエラー枠（outline、赤）とジャンプ強調（背景フラッシュ、青）の
//   2レイヤーに分離している。border は5マス区切りの太線やヒントグリッド
//   外周線の罫線描画に使われているため、強調表示でborderを上書きすると
//   罫線太さ・左右非対称指定と衝突しやすい。outlineと背景フラッシュは
//   レイアウト幅に影響しない別レイヤーなので、罫線設計に一切干渉しない。
// - ヒント編集UIはポップオーバー方式。行・列どちらも常に横長のテキスト入力
//   （"3 1 2" 形式）として表示するため、縦長で狭い列ヒントでも入力しやすい。
//   外側クリック・Escape・Enterで確定/キャンセルする。
// - 罫線太さの決定は isMajorLine / borderWeightPx に集約し、盤面・行ヒント・
//   列ヒントが同じ関数を同じ引数で呼ぶ。
// - スクロール同期: 盤面・行ヒント・列ヒント・交点ブロックは単一の
//   スクロールコンテナ内に置かれ、列ヒント行はsticky top-0、行ヒント列は
//   sticky left-0、交点ブロックはsticky top-0 left-0。スクロール位置は
//   React stateで管理しない。
// - 大規模盤面（100×100以上）: 1行/1列が1つのHintLineUnitなので、DOMノード数は
//   「行/列の数」分のみで、レンダリングコストを抑えている。
// ============================================================================
